...
// ✅ Pressions “autonomie” (synchronisées)
const p15Auto = document.getElementById("p15Auto");
const p5aAuto = document.getElementById("p5aAuto");
const p5bAuto = document.getElementById("p5bAuto");
...
// ---- Sync pressions entre onglets ----
function syncNeedToAuto(){
  if(!p15Auto || !p5aAuto || !p5bAuto) return;
  isSyncing = true;
  p15Auto.value = p15.value;
  p5aAuto.value = p5a.value;
  p5bAuto.value = p5b.value;
  isSyncing = false;
}

function syncAutoToNeed(){
  if(!p15Auto || !p5aAuto || !p5bAuto) return;
  isSyncing = true;
  p15.value = p15Auto.value;
  p5a.value = p5aAuto.value;
  p5b.value = p5bAuto.value;
  isSyncing = false;
}
...
function normalizeInputs(){
  ...
  // clamp aussi côté autonomie (si présent)
  if(p15Auto && p5aAuto && p5bAuto){
    p15Auto.value = clamp(parseInt(p15Auto.value || "0", 10), 0, 250);
    p5aAuto.value = clamp(parseInt(p5aAuto.value || "0", 10), 0, 250);
    p5bAuto.value = clamp(parseInt(p5bAuto.value || "0", 10), 0, 250);
  }
}
...
// ✅ Pressions modifiables sur l’onglet Autonomie + sync vers Besoin
if(p15Auto && p5aAuto && p5bAuto){
  [p15Auto, p5aAuto, p5bAuto].forEach(el => {
    el.addEventListener("input", () => {
      if(isSyncing) return;
      normalizeInputs();
      syncAutoToNeed();
